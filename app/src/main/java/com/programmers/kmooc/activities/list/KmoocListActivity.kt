...
class KmoocListActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.progressBar.visibility = (!viewModel.lectureList.hasObservers()).toVisibility()
        binding.lectureList.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                super.onScrolled(recyclerView, dx, dy)
                if (!recyclerView.canScrollVertically(1)) {
                    viewModel.next(viewModel.lectureList.value!!)
                }
            }
        })
    }
    ...
    private fun subscribeUi(adapter: LecturesAdapter) {
